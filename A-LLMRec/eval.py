...
def get_answers_predictions(file_path):
    ...
    llm_predictions = [] # list of llm predictions
    with open(file_path, 'r') as f:
        for line in f:
            if 'Answer:' == line[:len('Answer:')]: 
                answer = line.replace('Answer:', '').strip()[1:-1].lower() # remove quotation marks
                answers.append(answer) 
            if 'LLM:' == line[:len('LLM:')]: 
                llm_prediction = line.replace('LLM', '').strip().lower() # remove quotation marks
                try:
