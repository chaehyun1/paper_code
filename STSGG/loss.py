...
class RelationLossComputation(object):
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, gt_or_pseudo_list):
        ...
        """
        # relation_logits → 관계 예측 결과
        # refine_obj_logits → 객체 예측 결과
        # rel_labels → GT 관계 라벨 
        # fg_labels → GT 객체 라벨
        # gt_or_pseudo_list == 1 → pseudo label

        refine_obj_logits = refine_logits 

        relation_logits = cat(relation_logits, dim=0) # 하나의 텐서로 변환
        refine_obj_logits = cat(refine_obj_logits, dim=0)

        fg_labels = cat([proposal.get_field("labels") for proposal in proposals], dim=0) # proposals의 labels(GT)를 하나의 텐서로 변환
        rel_labels = cat(rel_labels, dim=0)
        
        if gt_or_pseudo_list is not None:
                # Loss for pseudo-labeled predicates
                # relation_logits[gt_or_pseudo_list == 1]: pseudo-label에 해당하는 model prediction \hat{p}^p_i
                # rel_labels[gt_or_pseudo_list == 1]: pseudo-label에 할당한 class \tilde{q}_i (one-hot vector: 값이 1인 위치가 해당 클래스의 라벨)
                # 해당 class는 predicates or bg
                # reduction='sum': 개별 손실을 합산하여 전체 손실로 변환
                loss_pseudo = F.cross_entropy(relation_logits[gt_or_pseudo_list == 1], 
                                              rel_labels[gt_or_pseudo_list == 1], reduction='sum') * (cfg.MODEL.ROI_RELATION_HEAD.STSGG_MODULE.BETA)
                
                # Loss for bg class
                # gt_or_pseudo_list == 0: annotated에 해당 (GT)
                # rel_labels == 0: bg class
                # relation_logits: \hat{p}^p_i
                # rel_labels: p^{bg}_i
                # 예측한 relation_logits과 rel_labels을 비교하여 loss 계산 
                loss_bg = F.cross_entropy(relation_logits[(gt_or_pseudo_list == 0) & (rel_labels == 0)], 
                                          rel_labels[(gt_or_pseudo_list == 0) & (rel_labels == 0)], reduction='sum')
                
                # Loss for annotated predicates
                # gt_or_pseudo_list == 0: annotated에 해당 (GT)
                # rel_labels > 0: predicates
                # relation_logits: \hat{p}^p_i
                # rel_labels: p_i
                loss_gt_rel = F.cross_entropy(relation_logits[(gt_or_pseudo_list == 0) & (rel_labels > 0)], 
                                              rel_labels[(gt_or_pseudo_list == 0) & (rel_labels > 0)], reduction='sum')
                
                n = (rel_labels >= 0).sum() # 전체 relation 개수 (bg + predicates)
                loss_relation = (loss_pseudo + loss_bg + loss_gt_rel) / n # 논문 식 (1)
        else:
            loss_relation = self.criterion_loss(relation_logits[rel_labels != -1], rel_labels[rel_labels!=-1].long())
            
        loss_refine_obj = self.criterion_loss(refine_obj_logits, fg_labels.long()) # 객체 분류 손실 계산
...
class WRelationLossComputation(object): # Weakly Supervised
    """
    Computes the loss for relation triplet.
    Also supports FPN
    """
    ...
    def __init__(
        ...
        if "STSGG" in cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR:
            self.st_coef = cfg.MODEL.ROI_RELATION_HEAD.STSGG_MODULE.BETA # 논문 식 1 참고
        else:
            self.st_coef = 1.0
...
def make_roi_relation_loss_evaluator(cfg):
    ...
    loss_evaluator = RelationLossComputation(
        ...
        cfg.MODEL.ROI_ATTRIBUTE_HEAD.ATTRIBUTE_BGFG_SAMPLE, # True
