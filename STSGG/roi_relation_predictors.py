...
class MotifPredictor(nn.Module):
    ...
    def __init__(self, config, in_channels):
        ...
        self.attribute_on = config.MODEL.ATTRIBUTE_ON # False
        ...
        # train_use_bias is to use bias in the training
        self.train_use_bias = config.MODEL.ROI_RELATION_HEAD.TRAIN_USE_BIAS # False
        # predict_use_bias is to use bias in the inference
        self.predict_use_bias = config.MODEL.ROI_RELATION_HEAD.PREDICT_USE_BIAS # True
        # load class dict
...
class MotifPredictor_STSGG(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.attribute_on = config.MODEL.ATTRIBUTE_ON # False
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES # 81
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES # 201
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES # 51
        ...
        if self.train_use_bias or self.predict_use_bias:
            # 빈번하게 등장하는 관계를 학습할 수 있도록 Frequency Bias 적용
            # 데이터셋에서 자주 등장하는 관계를 반영하여 모델 성능을 향상.
            self.freq_bias = FrequencyBias(config, statistics)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.attribute_on:
            ...
            obj_dists, obj_preds, edge_ctx, _ = self.context_layer(roi_features, proposals, logger) 
            # SGG 모델은 객체 간 관계를 고려하여 객체 클래스를 다시 보정.
            # edge_ctx: 객체 간 관계 특징
            
        # post decode
        edge_rep = self.post_emb(edge_ctx) # LSTM을 통해 인코딩된 객체 간 관계 특징을 Linear Layer에 통과
        edge_rep = edge_rep.view(edge_rep.size(0), 2, self.hidden_dim) # 관계 벡터(edge_ctx)를 분할하여 head/tail 객체 벡터 생성
        head_rep = edge_rep[:, 0].contiguous().view(-1, self.hidden_dim) 
        tail_rep = edge_rep[:, 1].contiguous().view(-1, self.hidden_dim)

        num_rels = [r.shape[0] for r in rel_pair_idxs] # 각 이미지당 관계 개수
        num_objs = [len(b) for b in proposals] # 각 이미지당 객체 개수
        assert len(num_rels) == len(num_objs)

        # 각 이미지에 대해 head/tail 객체 벡터를 분리
        # Head(주어), 관계(Predicate), Tail(목적어)
        head_reps = head_rep.split(num_objs, dim=0) 
        tail_reps = tail_rep.split(num_objs, dim=0)
        obj_preds = obj_preds.split(num_objs, dim=0)
        ...
        for pair_idx, head_rep, tail_rep, obj_pred in zip(rel_pair_idxs, head_reps, tail_reps, obj_preds):
            ...
            prod_reps.append( torch.cat((head_rep[pair_idx[:,0]], tail_rep[pair_idx[:,1]]), dim=-1) )
            pair_preds.append( torch.stack((obj_pred[pair_idx[:,0]], obj_pred[pair_idx[:,1]]), dim=1) )
        prod_rep = cat(prod_reps, dim=0)  # head와 tail의 관계 벡터를 합침. 관계를 예측할 때 사용하는 관계 특징 벡터
        pair_pred = cat(pair_preds, dim=0)  # head와 tail의 객체 클래스를 나타냄. 

        prod_rep = self.post_cat(prod_rep) 

        # 객체 간 관계를 예측할 때, 시각적 정보까지 고려
        if self.use_vision:
            if self.union_single_not_match:
                prod_rep = prod_rep * self.up_dim(union_features)
            else:
        ...
        rel_dists = self.rel_compress(prod_rep) # 최종 관계 예측 수행: 관계 예측 결과(logits) 생성

        if (self.train_use_bias and self.training) or (self.predict_use_bias and not self.training):
            rel_dists = rel_dists + self.freq_bias.index_with_labels(pair_pred.long()) 
            # 데이터셋의 관계 빈도를 기반으로 예측값을 보정
            # ex) "sitting on"이 "on"보다 자주 등장하면 "sitting on"의 확률을 증가시키는 방식
        obj_dists = obj_dists.split(num_objs, dim=0)
        rel_dists = rel_dists.split(num_rels, dim=0)
        ...
        if self.training:
            ...
            rel_pesudo_labels, gt_or_pseudo_list = self.ST_module(rel_pair_idxs, proposals, gt_label, rel_dists) # ST_SGG forward 함수 호출
            self.ST_module.update_class_threshold(rel_pesudo_labels, gt_or_pseudo_list) # 논문 식 (2)

        return obj_dists, rel_dists, add_losses, rel_pesudo_labels, gt_or_pseudo_list
        # obj_dists: 각 객체가 어떤 클래스인지 확률 분포(logits)로 반환.
        # rel_dists: 관계 예측 결과(logits)로 반환.
        # add_losses: 추가적인 loss로 반환.
        # rel_pesudo_labels: pseudo-label이 반영된 관계 레이블
        # gt_or_pseudo_list: ground-truth 또는 pseudo-label인지를 나타내는 리스트
...
def make_roi_relation_predictor(cfg, in_channels):
    func = registry.ROI_RELATION_PREDICTOR[cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR] # MotifPredictor_STSGG
