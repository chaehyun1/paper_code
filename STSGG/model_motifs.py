...
class LSTMContext(nn.Module):
    ...
    def obj_ctx(self, obj_feats, proposals, obj_labels=None, boxes_per_cls=None, ctx_average=False):
        # Context: 개별 객체 특징(Object Features)만을 고려하는 것이 아니라, 객체 간의 상호작용 관계까지 고려하는 것.
        """
        ...
        perm, inv_perm, ls_transposed = self.sort_rois(proposals) # Faster R-CNN이 탐지한 객체들을 Confidence Score 기준으로 정렬.
        # Pass object features, sorted by score, into the encoder LSTM
        obj_inp_rep = obj_feats[perm].contiguous()
        input_packed = PackedSequence(obj_inp_rep, ls_transposed)
        encoder_rep = self.obj_ctx_rnn(input_packed)[0][0] # NOTE: LSTM 인코더를 통해 객체 간 Context 학습.
        ...
        if (not self.training) and self.effect_analysis and ctx_average:
            ...
            # NOTE: Context를 반영한 객체 특징 벡터 생성
            decoder_inp = torch.cat((obj_inp_rep, encoder_rep), 1) # 기존 객체 특징(obj_inp_rep)과 LSTM 인코더가 학습한 문맥(encoder_rep)을 결합
        ...
        if self.mode != 'predcls':
            ...
            obj_dists, obj_preds = self.decoder_rnn(
                ...
                ) # NOTE: LSTM 디코더에서 Context를 반영하여 최종 객체 분류 확률(obj_dists)과 예측(obj_preds)을 생성.
        ...
        encoder_rep = encoder_rep[inv_perm] # Context를 학습한 후, 원래 객체 순서로 되돌림.
        # NOTE: encoder_rep = Contextual Features
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        # x: Faster R-CNN에서 추출한 객체 특징 (Object Features)
        # proposals: Faster R-CNN이 탐지한 객체들 (Bounding Box 포함)
        # rel_pair_idxs: 객체 간 관계 후보 쌍 (Object Pairs)
        # labels will be used in DecoderRNN during training (for nms)
        if self.training or self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
            obj_labels = cat([proposal.get_field("labels") for proposal in proposals], dim=0) # 예측된 proposal의 GT 객체 레이블 가져오기.
        else:
            obj_labels = None

        if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL: # GT 객체 레이블을 사용할 경우
            obj_embed = self.obj_embed1(obj_labels.long()) # GT 레이블을 임베딩
        else: # Faster R-CNN의 예측값을 사용할 경우
        ...
        pos_embed = self.pos_embed(encode_box_info(proposals)) # 객체의 공간적 정보 
        ...
        if all_average and self.effect_analysis and (not self.training):
            ...
            obj_pre_rep = cat((x, obj_embed, pos_embed), -1) # 객체의 최종 특징 벡터 = (Faster R-CNN 특징 + 객체 임베딩 + 공간 정보)
        ...
        # SGG 모델이 객체 간 Context를 고려하여 새로운 객체 분류(obj_dists) 수행.
        # obj_ctx: 객체 단위에서 context를 반영한 벡터 (Contextual Features)
        ...
        if (all_average or ctx_average) and self.effect_analysis and (not self.training):
            ...
            obj_rel_rep = cat((obj_embed2, x, obj_ctx), -1) # obj_embed2: 객체 임베딩, x: Faster R-CNN 특징, obj_ctx: 객체 단위 Contextual Features
            
        edge_ctx = self.edge_ctx(obj_rel_rep, perm=perm, inv_perm=inv_perm, ls_transposed=ls_transposed) 
        # obj_rel_rep을 LSTM에 입력하여 관계 예측을 위한 최종 관계(Context) 벡터 생성
        ...
        # obj_dists → 객체 분류 확률 ([num_objects, num_classes])
        # obj_preds → 최종 객체 예측 ([num_objects])
        # edge_ctx → 관계 예측을 위한 특징 
